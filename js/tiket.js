document.addEventListener('DOMContentLoaded', function() {
    ...
    // ==========================================
    // FITUR 1: TOTAL HARGA OTOMATIS TIKET
    // Lokasi: Halaman Beli Tiket
    // ==========================================
    const inputOrang = document.getElementById('input-orang');
    const teksTotal = document.getElementById('teks-total');
    
    if (inputOrang && teksTotal) {
        // HARGA SUDAH NAIK JADI 10.000 CAK!
    ...
    // ==========================================
    // FITUR 2: VALIDASI NO HP (Indikator Warna)
    // ==========================================
    function setupPhoneValidation(inputId, counterId) {
        ...
        if (input && counter) {
            input.addEventListener('input', function() {
                this.value = this.value.replace(/[^0-9]/g, ''); // Hapus selain angka
    ...
    // ==========================================
    // FITUR 3: PREVIEW FOTO BUKTI TRANSFER
    // ==========================================
    const buktiInput = document.getElementById('bukti-input');
    const uploadPreview = document.getElementById('upload-preview');
    const uploadPlaceholder = document.getElementById('upload-placeholder');

    if (buktiInput && uploadPreview && uploadPlaceholder) {
    ...
    // ==========================================
    // FITUR 4: KELOLA LIST SAMPAH (FORM PENGUNJUNG)
    // ==========================================
    ...
    if (sampahListContainer && sampahSummary) {
        ...
        // PERBAIKAN: Menggunakan .closest() agar klik di icon tetap terdeteksi!
        sampahListContainer.addEventListener('click', function(e) {
            
            // Logika Tombol Plus
            const btnPlus = e.target.closest('.btn-plus');
            if (btnPlus) {
                const inputQty = btnPlus.closest('.sampah-item').querySelector('.qty-input');
            ...
            if (btnMinus) {
                const inputQty = btnMinus.closest('.sampah-item').querySelector('.qty-input');
            ...
            const btnDel = e.target.closest('.m-del');
            if (btnDel) {
        ...
        // Event ketik manual di kolom angka
    ...
    // ==========================================
    // FITUR 5: SALIN NOMOR REKENING (COPY TO CLIPBOARD)
    // ==========================================
    const btnCopies = document.querySelectorAll('.btn-copy');
    if (btnCopies.length > 0) {
