...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_tiket'])) {
    ...
    if (isset($_POST['nama_anggota']) && is_array($_POST['nama_anggota'])) {
        ...
        foreach ($_POST['nama_anggota'] as $nama_ang) {

            $nama_ang_clean = trim($nama_ang);

            if (!empty($nama_ang_clean)) {

                $orang++; 

                $list_wa_anggota .= $no_urut . ". " . ucwords($nama_ang_clean) . "\n";
    ...
    if(!$link_gambar || strpos($link_gambar, 'http') === false) {
        ...
        echo "<script>alert('Gagal upload bukti transfer! Error: $err_msg'); window.history.back();</script>";
    ...
    if (mysqli_query($koneksi, $query)) {
        ...
        if (isset($_POST['nama_anggota']) && is_array($_POST['nama_anggota'])) {
            ...
            foreach ($_POST['nama_anggota'] as $nama_ang) {
                ...
                $nama_ang_clean = mysqli_real_escape_string($koneksi, trim($nama_ang));

                if (!empty($nama_ang_clean)) {

                    mysqli_query($koneksi, "INSERT INTO anggota (id_tiket, nama_anggota) VALUES ($id_baru, '$nama_ang_clean')");
...
// Pastikan variabel ini terisi sebelum dimasukkan ke string $pesan_wa

// Jika $jumlah_orang atau $total_harga kosong, sistem tidak akan menampilkan angkanya.



$jumlah_orang = isset($data_tiket['orang']) ? $data_tiket['orang'] : 0; 

$total_harga = $jumlah_orang * 10000;



        $pesan_wa = "🎟️ *E-TIKET TANCAK PANTI*\n\n";

        $pesan_wa .= "Halo, *" . $nama . "*! Terima kasih, tiket Anda telah terkonfirmasi.\n\n";

        $pesan_wa .= "📅 *Tanggal:* " . date('d-m-Y') . "\n";

        $pesan_wa .= "🏷️ *Kode Tiket:* `" . $kode_tiket . "`\n";

        $pesan_wa .= "📍 *Alamat:* " . $alamat . "\n";

        $pesan_wa .= "📌 *DAFTAR ROMBONGAN:*\n" . $list_wa_anggota . "\n";

        $pesan_wa .= "━━━━━━━━━━━━━━\n";

        $pesan_wa .= "⚠️ *INSTRUKSI LOKET:*\n";

        $pesan_wa .= "Harap tunjukkan pesan ini kepada petugas loket saat tiba di loket.\n\n";

        $pesan_wa .= "🔗 *Buka E-Tiket:* tancak-panti.mif.myhost.id/cek_tiket?id=" . $id_baru . "\n\n";

        $pesan_wa .= "Sampai jumpa di Air Terjun Tancak! 🙌💚";
...
<main class="flex-1 pb-16 px-4" style="padding-top: var(--header-height);">
        ...
        <div id="tiket-card" class="max-w-[620px] mx-auto bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="relative flex border-b border-gray-100 bg-white">
                ...
                <a href="/tiket" class="flex-1 py-4 flex justify-center items-center gap-2 font-bold text-[14px] relative z-10 text-white transition-colors duration-300">
                ...
                <a href="/cek_tiket" id="link-to-cek" class="flex-1 py-4 flex justify-center items-center gap-2 font-semibold text-[14px] relative z-10 text-gray-500 hover:text-[#1a3326] transition-colors duration-300">
                            ...
                            <div class="mb-8">
                                ...
                                <div class="border border-gray-200 rounded-[12px] p-2 bg-gray-50 flex justify-center w-full">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <span class="text-xs text-red-500 font-bold p-4">Gambar gagal dimuat.</span>
                ...
                <form action="" method="POST" enctype="multipart/form-data" id="form-beli-tiket">
                    ...
                    <div class="space-y-5 mb-10">
                        ...
                        <div class="grid grid-cols-3 items-center gap-4">
                            ...
                            <input type="text" name="alamat" required placeholder="Kecamatan / Desa" class="col-span-2 bg-[#f8faf9] border border-gray-200 rounded-[12px] px-4 py-3 text-[14px] outline-none focus:border-[#2d6a4f] transition-all capitalize">

                        </div>

                        

                        <div class="grid grid-cols-3 items-start gap-4 mb-5">

                            <label class="font-bold text-[#1a3326] text-[14px] mt-3">Anggota Rombongan</label>

                            <div class="col-span-2">

                                <div class="flex gap-2">

                                    <input type="text" id="input-anggota-baru" placeholder="Ketik nama anggota..." class="w-full bg-[#f8faf9] border border-gray-200 rounded-[12px] px-4 py-3 text-[14px] outline-none focus:border-[#2d6a4f] capitalize">

                                    <button type="button" id="btn-tambah-anggota" class="bg-[#1a3326] text-white px-4 rounded-[12px] text-[13px] font-bold hover:bg-[#12241b] transition-colors whitespace-nowrap">+ Tambah</button>

                                </div>

                                <div id="anggota-list-container" class="space-y-2 mt-3"></div>

                                <div class="flex justify-between items-center mt-3 px-1 border-t border-gray-100 pt-3">

                                    <span id="teks-jumlah-orang" class="text-[11px] text-gray-400">Total: 1 Orang (Kepala Saja)</span>

                                    <span id="teks-total" class="text-[13px] font-bold text-[#006A6A]">Total: Rp 10.000</span>
                        ...
                        <div class="grid grid-cols-3 items-start gap-4">
                            ...
                            <div class="col-span-2 relative">
                                ...
                                <input type="text" id="input-telp1" name="no_telp" required minlength="11" maxlength="15" oninput="this.value = this.value.replace(/[^0-9]/g, '')" placeholder="No. WA Aktif (min. 11 digit)" class="w-full bg-[#f8faf9] border border-gray-200 rounded-[12px] px-4 py-3 text-[14px] outline-none pr-14 transition-all">
                        ...
                        <div class="grid grid-cols-3 items-start gap-4">
                            ...
                            <div class="col-span-2 relative">
                                ...
                                <input type="text" id="input-telp2" name="no_darurat" required minlength="11" maxlength="15" oninput="this.value = this.value.replace(/[^0-9]/g, '')" placeholder="No. Orang Terdekat (min. 11 digit)" class="w-full bg-[#f8faf9] border border-gray-200 rounded-[12px] px-4 py-3 text-[14px] outline-none pr-14 transition-all">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-100">
                        ...
                        <div class="px-6 md:px-10"> 
                            ...
                            <button type="submit" name="submit_tiket" id="btn-submit-tiket" class="w-full bg-[#1b3d2f] text-white font-semibold text-[15px] h-[58px] rounded-[12px] flex items-center justify-center gap-2 hover:bg-[#122b21] transition-all shadow-sm active:scale-[0.98]">
    ...
    <div id="modal-sukses-beli" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 hidden backdrop-blur-sm transition-all duration-300">
        ...
        <div class="bg-white rounded-[24px] p-8 max-w-[380px] w-full text-center shadow-2xl transform scale-95 transition-transform duration-300" id="modal-card">
            ...
            <div class="w-20 h-20 bg-green-50 rounded-full flex items-center justify-center mx-auto mb-5 border-[4px] border-green-100">
                ...
                <svg class="w-10 h-10 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"></path></svg>
            ...
            <div class="flex flex-col gap-3">
                ...
                <a href="/cek_tiket" class="w-full bg-[#1a3326] hover:bg-[#12241b] text-white py-3.5 rounded-[14px] text-[14px] font-bold shadow-md transition-all flex items-center justify-center gap-2">

                    Cek E-Tiket Sekarang

                </a>

                <button type="button" id="btn-tutup-modal" class="w-full bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 py-3.5 rounded-[14px] text-[14px] font-bold transition-all">
